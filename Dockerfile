...
RUN DEBIAN_FRONTEND=noninteractive apt-get update && apt-get install -y sed cpio curl tar zstd libssl-dev openssl libxml2 which bc gawk perl diffutils make python3 xz-utils bison flex git unzip && \
 apt autoremove && apt clean && rm -rf /usr/share/man/* && rm -rf /var/lib/apt/lists/* && rm -rf /var/lib/apt/archives/* && rm -rf /var/lib/apt/caches/* && rm -rf /build/tc/share && rm -rf /build/tc/lib/cmake && rm -f /build/tc/lib/libclang-cpp.so.18git && rm -f /build/tc/lib/LLVMgold.so
